...
test.describe('MP staging login', () => {
  ...
  test('failed login shows the current incorrect-credentials message @dg', async () => {
  ...
  test('no email keeps sign in disabled when the email field is blank @dg', async () => {
    await loginPage.login('', 'anypassword', 'disabled')
    await loginPage.assertSignInDisabled()
  })

  test('no password shows the current incorrect-credentials message @dg', async () => {
    await loginPage.login('any@example.com', '', 'error')
    await loginPage.assertErrorMessageContains(incorrectCredentialsMessage)
  })

  test('login loads the dashboard and shows the current MP user in the profile menu @smoke @dg', async ({ page }) => {
